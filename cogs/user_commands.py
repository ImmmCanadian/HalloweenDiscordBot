...
class UserCommands(commands.Cog):
    ...
    async def profile(self, interaction: discord.Interaction):
        ...
        async with asqlite.connect('./database.db') as connection:
                async with connection.cursor() as cursor:
                    await cursor.execute(f'SELECT candy, bank, roles, pets, bg_color, bg_image FROM Users WHERE id = ?', (user_id,))
                    db_result = await cursor.fetchone()
        
        candy, bank, roles_json, pets_json, bg_color, bg_image = db_result
        roles = json.loads(roles_json) if roles_json else []
        pets = json.loads(pets_json) if pets_json else []
        
        # Default to purple if no color is set
        if not bg_color:
            bg_color = "purple"
        
        # Create the profile image
        profile_image = await self.create_profile_image(interaction.user, candy, bank, roles, pets, bg_color, bg_image)
    ...
    async def create_profile_image(self, user, candy, bank, roles, pets, bg_color="purple", bg_image=None):
        ...
        # Try to use custom background image if provided
        if bg_image:
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(bg_image, headers=headers, timeout=5) as response:
                        if response.status == 200:
                            image_data = await response.read()
                            bg_img = Image.open(BytesIO(image_data)).convert('RGB')
                            ...
                            # Fall back to color gradient if image fails
                            self.draw_gradient_background(draw, width, height, bg_color)
            except Exception as e:
                print(f"Failed to load background image: {e}")
                # Fall back to color gradient
                self.draw_gradient_background(draw, width, height, bg_color)
        else:
            # Use color gradient
            self.draw_gradient_background(draw, width, height, bg_color)
        ...
        # Balance section - without emojis in header
        ...
        # Pets section with increased spacing
        pets_y = total_y + 60
        self.draw_section_header(draw, "PETS:", 50, pets_y, header_font)
        
        pets_text = "No pets yet"
        if pets:
            if len(pets) <= 3:
                pets_text = ", ".join(pets)
            else:
                pets_text = ", ".join(pets[:3]) + f" +{len(pets)-3} more"
        
        pets_content_y = pets_y + 45
        draw.text((70, pets_content_y), pets_text, fill='white', font=text_font)
        
        # Roles section with increased spacing
        roles_y = pets_content_y + 60
        ...
        # draw.rectangle([20, 20, width-20, height-20], outline=(100, 100, 100), width=1)
